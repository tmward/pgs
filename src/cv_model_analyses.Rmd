...
plot_confusion_matrix <- function(df, groundtruth, prediction, title = "Confusion Matrix") {
    df %>%
...
```{r}
only_cf <- plot_confusion_matrix(results, gt, pgs_only, "PGS only CNN") +
...
combo_cf <- plot_confusion_matrix(
    ...
    theme(legend.position = "none")
surg2_cf <- plot_confusion_matrix(
        results,
        gt,
        pgs_surg2,
        "Second Surgeon PGS"
